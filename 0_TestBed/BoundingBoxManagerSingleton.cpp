...
void BoundingBoxManagerSingleton::AddBoxToRenderList(String a_sInstanceName)
{
	...
	if(a_sInstanceName == "ALL")
	{
		for(int nBox = 0; nBox < m_nBoxs; nBox++)
		{
			...
			m_lBox[nBox]->AddAABBoxToRenderList(m_lMatrix[nBox], m_lColor[nBox], false);
		}
	}
	else
	{
...
void BoundingBoxManagerSingleton::CalculateCollision(void)
{
	...
	for(int nBox = 0; nBox < m_nBoxs; nBox++)
	{
		...
		lCentroid.push_back(static_cast<vector3>(m_lMatrix[nBox] * vector4(m_lBox[nBox]->GetCentroidAABB(), 1.0f)));
	}
	...
	for(int i = 0; i < m_nBoxs - 1; i++)
	{
		for(int j = i + 1; j < m_nBoxs; j++)
		{
			...
			//?
			//vector3 v1Minno = static_cast<vector3>(m_lMatrix[i] * vector4(m_lBox[i]->GetMaximumOBB(),1));
